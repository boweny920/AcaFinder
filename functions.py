...
def Aca_HMM_search(aca_candidate_file,published_acaHMM,threads,hmm_outfile,evalue_cut_off,outdir,coverage_cutoff,gff_file,fna_file,complete_CRISPR_Cas_systems,prophage_regions):
    ...
    for line in open(hmm_outfile).readlines():
        ...
        if "#" not in line[0]:
            ...
            if coverage > coverage_cutoff and int(line[2]) < 150: # Coverage and protein length filter
    ...
    if len(AcaPub_Pro_lst_dic) > 0:
        ...
        for index, row in df.iterrows():
            ...
            else: prophage_with_Acas.append(np.nan)
        ...
        for index, row in df.iterrows():
            if complete_CRISPR_Cas_systems is not None:
                ...
                Complete_CRISPR_Cas_inContig_list.append(np.nan)
